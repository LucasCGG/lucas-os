...
export class PauseScreen {
    ...
    onPress(mx: number, my: number): PauseAction | null {
        const clicked = this.buttons.find((button) => button.contains(mx, my));
        ...
        for (let i = 0; i < this.barRects.length; i++) {
            if (this.hits(this.barRects[i], mx, my)) {
                this.draggingRow = i;
    ...
    onDrag(mx: number, _my: number): void {
        if (this.draggingRow !== -1) {
            this.applyValue(this.draggingRow, mx);
        }
    }
    ...
    private applyValue(rowIndex: number, mx: number): void {
        const r = this.barRects[rowIndex];

        const value = Math.max(0, Math.min(1, (mx - r.x) / r.w));

        this.rows[rowIndex].set(value);
    }
    ...
    draw(ctx: CanvasRenderingContext2D, width: number, height: number): void {
        ...
        ctx.font = this.compact ? "bold 20px Arial" : "bold 44px Arial";
        ctx.fillText(PauseScreen.TITLE, width / 2, titleY);
        ...
        for (let i = 0; i < this.rows.length; i++) {
            ...
            const value = this.rows[i].get();

            ctx.textAlign = "right";
            ctx.fillStyle = "#e8e8ff";

            ctx.fillText(this.rows[i].label, barX - 14, y + barH - 2);
            ...
            ctx.fillStyle = this.draggingRow === i ? "#ffffff" : "#c8ffd4";

            ctx.beginPath();

            ctx.arc(knobX, y + barH / 2, barH * 0.6, 0, Math.PI * 2);
        ...
        const buttonStartY = startY + this.rows.length * rowGap + (this.compact ? 14 : 30);

        const x = width / 2 - bw / 2;

        this.buttons.forEach((button, i) => {
            button.setRect(x, buttonStartY + i * (bh + gap), bw, bh);
            button.draw(ctx);
        });

        ctx.fillStyle = "#9aa0aa";

        ctx.font = this.compact ? "10px Arial" : "16px Arial";
