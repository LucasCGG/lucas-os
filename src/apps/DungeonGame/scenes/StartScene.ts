...
export class StartScene extends GameScene {
    ...
    private layoutButtons(width: number, height: number): void {
        ...
        const startY = this.compact ? height - totalHeight - 28 : height * 0.5 - totalHeight / 6;

        const x = width / 2 - bw / 2;

        this.buttons.forEach((b, i) => {
            b.setRect(x, startY + i * (bh + gap), bw, bh);
        });
    }
    ...
    update(deltaTime: number): void {
        ...
        if (this.keys.isKeyJustPressed("Enter") || this.keys.isKeyJustPressed("Space")) {
            this.activate("start");
        }
    ...
    render(ctx: CanvasRenderingContext2D): void {
        ...
        ctx.fillText(GAME_TITLE, width / 2, titleY + bob);

        ctx.fillStyle = "#9aa0aa";
        ctx.font = this.compact ? "11px monospace" : "18px monospace";
        ctx.fillText(SUBTITLE, width / 2, titleY + subtitleGap + bob);
        ...
        if (!this.compact) {
            ...
            ctx.fillText("Enter to start", width / 2, height - 24);
        }
