...
interface WindowProps {
    ...
    __animHidden?: boolean;
    __animDisabled?: boolean;
}
...
export const Window = ({
    ...
    return (
        <Rnd
            size={size}
            position={position}
            disableDragging={isFullscreen || __animDisabled}
            enableResizing={!isFullscreen && !__animDisabled}
            minWidth={300}
            minHeight={200}
            ...
            style={{
                zIndex,
                opacity: __animHidden ? 0 : 1,
                visibility: __animHidden ? "hidden" : "visible",
        ...
        >
            {useMemo(
                () => (
                    <div className="window-titlebar text-md flex cursor-move items-center justify-between bg-sidebar px-3 py-2 font-medium text-text-muted" style={{zIndex:"2", position:"relative"}}>
